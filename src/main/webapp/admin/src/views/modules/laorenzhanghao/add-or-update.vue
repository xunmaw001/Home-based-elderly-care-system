  ...
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          laorennianling: [
                { required: true, message: '老人年龄不能为空', trigger: 'blur' },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `laorenzhanghao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.laorenzhanghaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
