<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="生活用品" prop="shenghuoyongpin">
          <el-input v-model="ruleForm.shenghuoyongpin" 
              placeholder="生活用品" clearable  :readonly="ro.shenghuoyongpin"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="生活用品" prop="shenghuoyongpin">
              <el-input v-model="ruleForm.shenghuoyongpin" 
                placeholder="生活用品" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='shenghuoyongpin'){
            this.ruleForm.shenghuoyongpin = obj[o];
	    this.ro.shenghuoyongpin = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `shenghuoyongpinxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shenghuoyongpinxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shenghuoyongpinxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
