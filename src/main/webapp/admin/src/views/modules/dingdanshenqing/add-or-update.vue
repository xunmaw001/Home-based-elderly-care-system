<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="检查检验项目" prop="jianchajianyanxiangmu">
          <el-select v-model="ruleForm.jianchajianyanxiangmu" placeholder="请选择检查检验项目">
            <el-option
                v-for="(item,index) in jianchajianyanxiangmuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="维修服务" prop="weixiufuwu">
          <el-select v-model="ruleForm.weixiufuwu" placeholder="请选择维修服务">
            <el-option
                v-for="(item,index) in weixiufuwuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.laorenxingbie!=''&&json.laorenxingbie){
              		this.ruleForm.laorenxingbie = json.laorenxingbie
		}
		if(json.laorennianling!=''&&json.laorennianling){
              		this.ruleForm.laorennianling = json.laorennianling
		}
		...
		if(json.lianxidizhi!=''&&json.lianxidizhi){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jianchajianyanxiangmu/jianchajianyanxiangmu`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jianchajianyanxiangmuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            ...
            this.$http({
              url: `option/weixiufuwu/weixiufuwu`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.weixiufuwuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dingdanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dingdanshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
